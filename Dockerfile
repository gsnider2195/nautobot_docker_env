...
# podman run -it -v $XDG_RUNTIME_DIR/podman/podman.sock:/var/run/docker.sock -v .:$(pwd) -w $(pwd) --net=host --restart=unless-stopped --name=devenv devenv

# to use with docker:
# docker run -it -v /var/run/docker.sock:/var/run/docker.sock -v .:$(pwd) -w $(pwd) --net=host --restart=unless-stopped --name=devenv devenv
...
RUN pip install --no-cache-dir -r invoke
